...
def cargar_programas():
    try:
        ...
        messagebox.showerror("Error de Base de Datos", "Error al cargar programas desde la base de datos.")
...
def modificar_programa():
    try:
        ...
        conn = conectar_db()
        cursor = conn.cursor()
        cursor.execute("UPDATE programas SET nombre = ? WHERE id = ?", nuevo_nombre, id_programa)
        conn.commit()
        conn.close()
...
# Interfaz
app = ctk.CTk()
app.title("Gestión de Programas")
...
tree_programas = ttk.Treeview(frame, columns=("ID", "Nombre"), show="headings", height=10)
